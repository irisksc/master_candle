...
def run_ultimate_excel_backtest():
    ...
    print(f"\n" + "=" * 75)
    print(f"▶ 롱텐 엑셀 마스터 리포트 (보유종목 분리 및 컬러 서식) 생성 중... (총 {len(unique_codes)}개 종목)")
    print("=" * 75)
        ...
        for current_date, price_row in df_prices.iterrows():
            ...
            if is_holding:
                ...
                if added_this_week:
                    ...
                    hist_str = f"{buy_stage}차({date_short}) {int(curr_close)} 평단 {int(avg_price)}"
                    buy_history_list.append(hist_str)

                    detail_data.append({
                        '종목명': stock_name, '날짜': date_str, '액션': f'└─ {buy_stage}차 추매',
                        '평단가': int(avg_price), '상세내용': f"종가 {int(curr_close):,}원으로 물타기"
                    })
    ...
    # [추가됨] 미청산(보유중) 전용 시트 (수익률이 낮은 순 = 손실이 큰 순으로 정렬)
    ...
    # 엑셀 저장 및 서식 적용
    # ---------------------------------------------------------
    fill_profit = PatternFill(start_color="FFCCCC", end_color="FFCCCC", fill_type="solid")  # 연한 빨강
    fill_loss = PatternFill(start_color="CCE5FF", end_color="CCE5FF", fill_type="solid")  # 연한 파랑
    fill_breakeven = PatternFill(start_color="E6E6E6", end_color="E6E6E6", fill_type="solid")  # 연한 회색
    fill_holding = PatternFill(start_color="FFFFCC", end_color="FFFFCC", fill_type="solid")  # 연한 노랑

    fill_alt1 = PatternFill(start_color="FFFFFF", end_color="FFFFFF", fill_type="solid")  # 흰색
    fill_alt2 = PatternFill(start_color="F2F9FF", end_color="F2F9FF", fill_type="solid")  # 연한 하늘색
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        ...
        df_holding.to_excel(writer, sheet_name='미청산(보유중)', index=False)  # [추가됨] 보유중 시트 저장
        df_detail.to_excel(writer, sheet_name='상세로그', index=False)

        # 1. 요약 시트 3개에 대해 '결과' 컬럼 색상 칠하기
    ...
    print("\n" + "=" * 75)
    print(f"■ 마스터 백테스팅 완료! [미청산(보유중)] 시트가 포함된 최종 엑셀이 생성되었습니다.")
    print("=" * 75)

...
def main():
        ...
        for code, stock in portfolio.items():
            ...
            if s_open >= target_exit_price:
                trade_logs.append({
                    ...
                    "기준봉일자": signal_date, "매수일자": stock["buy_date"], "매수단가": int(old_avg_price),
                    "매도일자": current_date, "매도단가": int(s_open), "추매경로기록": " ".join(stock["history_logs"])
                })
            ...
            if s_high >= target_exit_price:
                trade_logs.append({
                    ...
                    "최종투자금": stock["total_spent"], "수익금": profit, "보유일수": stock["holding_days"],
                    "기준봉일자": signal_date, "매수일자": stock["buy_date"], "매수단가": int(stock["avg_price"]),
                    "매도일자": current_date, "매도단가": int(target_exit_price), "추매경로기록": " ".join(stock["history_logs"])
                })
                all_target_audit_logs[(code, signal_date)].update({
                    "최종상태": f"매매완료({exit_type_base.split('(')[0]})", "최초매수일": stock["buy_date"], "최종매도일": current_date,
                    "총투자금액": stock["total_spent"], "수익금": profit
        ...
        for (code, s_date), signal in pending_signals.items():
            ...
            if d_low <= p_33_price:
                ...
                portfolio[code] = {
                    ...
                    "history_logs": history_list, "signal_date": s_date
                }
    ...
    for code, stock in portfolio.items():
        trade_logs.append({
            ...
            "기준봉일자": stock["signal_date"], "매수일자": stock["buy_date"], "매수단가": int(stock["avg_price"]),
            "매도일자": "-[보유중]", "매도단가": 0, "추매경로기록": " ".join(stock["history_logs"])
        })
    ...
    for (code, s_date), audit in all_target_audit_logs.items():
        if "미체결" in audit["최종상태"] or "매수제외" in audit["최종상태"]:
            trade_logs.append({
                ...
                "최종투자금": 0, "수익금": 0, "보유일수": 0, "기준봉일자": s_date,
                "매수일자": "-", "매수단가": 0, "매도일자": "-", "매도단가": 0,
    ...
    # -------------------------------------------------------------------------
    # [신규 통합] 보유중 데이터 추가 연도별 다차원 통계 세션
    # -------------------------------------------------------------------------
    df_yearly_summary = pd.DataFrame()
    if not df_trade_logs.empty:
        df_trade_logs['연도'] = df_trade_logs['기준봉일자'].astype(str).str.replace('-', '').str[:4]
        years = sorted([y for y in df_trade_logs['연도'].unique() if y.isdigit() and len(y) == 4])
        yearly_stats = []

        for y in years:
            df_y = df_trade_logs[df_trade_logs['연도'] == y]
            cand_count = len(df_y)

            p_count = df_y['청산유형'].str.contains('익절청산').sum()
            b_count = df_y['청산유형'].str.contains('본절마감\(일반\)').sum()
            t_count = df_y['청산유형'].str.contains('본절마감\(장중동시터치\)').sum()
            h_count = df_y['청산유형'].str.contains('보유중').sum()  # 추가됨

            p_ratio = p_count / cand_count if cand_count > 0 else 0
            b_ratio = b_count / cand_count if cand_count > 0 else 0
            t_ratio = t_count / cand_count if cand_count > 0 else 0
            h_ratio = h_count / cand_count if cand_count > 0 else 0  # 추가됨

            df_entered = df_y[~df_y['청산유형'].str.contains('미체결|매수제외')]
            ent_count = len(df_entered)

            avg_hold = df_entered['보유일수'].mean() if ent_count > 0 else 0
            avg_add = df_entered['추매경로기록'].astype(str).apply(lambda x: x.count('추매가')).mean() if ent_count > 0 else 0

            yearly_stats.append({
                "연도": f"{y}년", "후보건수": cand_count,
                "익절청산건수": p_count, "익절청산비율": p_ratio,
                "본절마감건수": b_count, "본절마감비율": b_ratio,
                "본절마감(장중동시터치)건수": t_count, "본절마감(장중동시터치)비율": t_ratio,
                "보유중건수": h_count, "보유중비율": h_ratio,  # 추가됨
                "평균보유일수": avg_hold, "평균추매횟수": avg_add
            })

        total_cand = len(df_trade_logs)
        total_p = df_trade_logs['청산유형'].str.contains('익절청산').sum()
        total_b = df_trade_logs['청산유형'].str.contains('본절마감\(일반\)').sum()
        total_t = df_trade_logs['청산유형'].str.contains('본절마감\(장중동시터치\)').sum()
        total_h = df_trade_logs['청산유형'].str.contains('보유중').sum()  # 추가됨

        total_p_ratio = total_p / total_cand if total_cand > 0 else 0
        total_b_ratio = total_b / total_cand if total_cand > 0 else 0
        total_t_ratio = total_t / total_cand if total_cand > 0 else 0
        total_h_ratio = total_h / total_cand if total_cand > 0 else 0  # 추가됨

        df_tot_entered = df_trade_logs[~df_trade_logs['청산유형'].str.contains('미체결|매수제외')]
        tot_ent_count = len(df_tot_entered)
        tot_hold = df_tot_entered['보유일수'].mean() if tot_ent_count > 0 else 0
        tot_add = df_tot_entered['추매경로기록'].astype(str).apply(
            lambda x: x.count('추매가')).mean() if tot_ent_count > 0 else 0

        yearly_stats.append({
            "연도": "전체 총계", "후보건수": total_cand,
            "익절청산건수": total_p, "익절청산비율": total_p_ratio,
            "본절마감건수": total_b, "본절마감비율": total_b_ratio,
            "본절마감(장중동시터치)건수": total_t, "본절마감(장중동시터치)비율": total_t_ratio,
            "보유중건수": total_h, "보유중비율": total_h_ratio,  # 추가됨
            "평균보유일수": tot_hold, "평균추매횟수": tot_add
        })
        df_yearly_summary = pd.DataFrame(yearly_stats)
    ...
    with pd.ExcelWriter(EXCEL_OUTPUT_PATH, engine="openpyxl") as writer:
        df_summary.to_excel(writer, sheet_name="종합요약통계", index=False, startrow=0)
        if not df_yearly_summary.empty:
            df_yearly_summary.to_excel(writer, sheet_name="종합요약통계", index=False, startrow=6)
        ...
        for sheet_name in workbook.sheetnames:
            ...
            if sheet_name == "종합요약통계":
                ...
                for r_idx in range(2, 6):
                    ...
                    worksheet.cell(row=r_idx, column=2).alignment = align_center

                if not df_yearly_summary.empty:
                    # 열 확장: 12개 열에 대한 헤더 서식 지정
                    ...
                    for r_idx in range(8, 8 + len(df_yearly_summary)):
                        worksheet.cell(row=r_idx, column=1).alignment = align_center
                        worksheet.cell(row=r_idx, column=1).font = font_body

                        # 건수 데이터 서식 지정 (단위 콤마 처리) -> 열 인덱스: [2, 3, 5, 7, 9]
                        for c_idx in [2, 3, 5, 7, 9]:
                            cell = worksheet.cell(row=r_idx, column=c_idx)
                            cell.alignment = align_right;
                            cell.number_format = '#,##0';
                            cell.font = font_body

                        # 비율 데이터 서식 지정 (백분율 처리) -> 열 인덱스: [4, 6, 8, 10]
                        ...
                        # 평균 데이터 서식 지정 (소수점 1자리 처리) -> 열 인덱스: [11, 12]
                        for c_idx in [11, 12]:
                            cell = worksheet.cell(row=r_idx, column=c_idx)
                            cell.alignment = align_right;
                            cell.number_format = '0.0';
    ...
    print("\n" + "=" * 115)
    print(f" [완료] 연도별 세부 통계 (보유중 내역 추가) 완료 -> {EXCEL_OUTPUT_PATH}")
    print("=" * 115)
